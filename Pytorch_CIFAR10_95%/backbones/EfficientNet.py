...
def swish(x):
    # swish激活函数由Google提出，swish(x) = x * σ(x)
    # 用于MBConv中的SE模块
    return x * x.sigmoid()
...
def drop_connect(x, drop_ratio):
    # 作用于权重
    # 以1-p概率将节点中的每个与其相连的输入权重清0
    keep_ratio = 1.0 - drop_ratio
    mask = torch.empty([x.shape[0], 1, 1, 1], dtype=x.dtype, device=x.device)
    # 按照伯努利分布(bernoulli_)以概率p(keep_ratio)生成一个Mask二进制矩阵
...
class EfficientNet(nn.Module):
    def __init__(self, cfg, num_classes=10):
        ...
        # cfg['out_channels'][-1]。获取EfficientNet最后一层的输出通道数
